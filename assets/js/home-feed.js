/* =========================================================
   ...
   e preenche as seções da home. Se não houver posts de uma
   categoria, a home mantém o conteúdo estático de exemplo
   (Notícias) ou mantém a seção escondida (Ficção/Reviews).
   ========================================================= */
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Últimas notícias (substitui o bloco de destaque + laterais)
    const noticias = posts.filter(p => p.categoria === "Notícias").slice(0, 4);
    const newsGrid = document.getElementById("home-news-grid");
    if (newsGrid && noticias.length) {
        ...
        newsGrid.innerHTML = `
            ...
            <div class="news-side">
                ${rest.map(p => `<article class="mini-card" data-animate>
    ...
    function fillCategorySection(categoriaLabel, sectionId, gridId) {
        const items = posts.filter(p => p.categoria === categoriaLabel).slice(0, 3);
